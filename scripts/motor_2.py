...
class motor_command():
    def __init__(self):#初期化
        ...
        self.Enable = True
        self.pin_num_PWM = 27#pinの名前
        self.pin_num_RotationDirect = 17
        self.pin_num_Enable = 4
        self.pin_num_Stop = 22
        self.freq = 50 # Hz (PWM のパルスを一秒間に 50 個生成)
        #ノード立ち上げ（ROS）
        rospy.init_node('motor2_control')
        rospy.Subscriber('motor2/cmd_vel', Int64, self.callback)
        rospy.loginfo('Ready to receive motor2 command')
